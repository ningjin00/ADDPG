...
from DroneFlightEnv import DroneFlightEnv
...
class DDPG:
    ...
    def take_action(self, state):
        ...
        if np.random.uniform() < self.epsilon:
            action = torch.tensor([np.random.uniform(-50,50,2)])
        else:
    ...
    def update(self, transition_dict):
        ...
        actions = torch.cat(transition_dict['actions'], dim=0).float()
        # print(actions)
...
if __name__ == '__main__':
    ...
    state_dim = env.observation_space.shape[0]   # 二维空间
    action_dim = env.action_space.shape[0]
    action_bound = env.action_space.high[0]  # 动作最大值
    agent = DDPG(state_dim, action_dim, state_dim + action_dim, hidden_dim, False,
                 action_bound, sigma, actor_lr, critic_lr, tau, gamma, device,epsilon)

    return_list = rl_utils.train_off_policy_agent(env, agent, num_episodes,
                                                  replay_buffer, minimal_size,
                                                  batch_size)
    # return_list = []
    # for p in return_list1:
    #     return_list.append(p / 28)

    # np.save('return7.npy', return_list)

    episodes_list = list(range(len(return_list)))
    plt.plot(episodes_list, return_list)
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('DDPG on {}'.format("UAV-deployment"))
    plt.show()
    #
